...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            is_relevant.append(overlap >= relevance_threshold)

        num_relevant = sum(is_relevant)
        if num_relevant == 0:
            return 0.0

        precision_sum = 0.0
        relevant_so_far = 0
        for k, relevant in enumerate(is_relevant, start=1):
            if relevant:
                relevant_so_far += 1
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Answer: {answer}\n"
            f"Rubric:\n{criteria_text}\n\n"
            "Score each criterion from 0.0 to 1.0 and respond with a JSON "
            "object mapping criterion name to score."
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        def entry_avg(entry: dict[str, Any]) -> float | None:
            scores = entry.get("scores", {})
            if not scores:
                return None
            return sum(scores.values()) / len(scores)

        entry_avgs = [a for a in (entry_avg(e) for e in scores_batch) if a is not None]

        # Positional bias: does the response presented first in each pair
        # score systematically higher than the one presented second?
        positional_bias = False
        first_position = entry_avgs[0::2]
        second_position = entry_avgs[1::2]
        if first_position and second_position:
            first_mean = sum(first_position) / len(first_position)
            second_mean = sum(second_position) / len(second_position)
            positional_bias = first_mean > second_mean + 0.15

        leniency_bias = False
        severity_bias = False
        if entry_avgs:
            overall_avg = sum(entry_avgs) / len(entry_avgs)
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        avg_context_recall = avg(recall_scores) if recall_scores else None
        avg_context_precision = avg(precision_scores) if precision_scores else None
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        baseline_avgs = {m: avg(baseline_results, m) for m in metrics}

        regressions = [
            m for m in metrics if baseline_avgs[m] - new_avgs[m] > 0.05
        ]
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        lowest = low_scores[0] if low_scores else min(scores, key=scores.get)
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, failure in enumerate(failures):
            ...
            if suggestions:
                suggestion = suggestions[i] if i < len(suggestions) else suggestions[-1]
            else:
                suggestion = ""
            rows.append(f"| {failure_id} | {failure_type} | {root_cause} | {suggestion} | Open |")
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            suggestions.append(
                "Implement a hallucination checker to filter unsupported claims and enforce citation-based grounding"
            )
        if categories.get("irrelevant"):
            suggestions.append(
                "Improve prompt clarity and intent detection so answers stay on-topic"
            )
        ...
        if categories.get("off_topic"):
            suggestions.append(
                "Review intent routing/classification to reduce off-topic responses"
            )
        if categories.get("refusal"):
            suggestions.append(
                "Relax overly strict guardrails so the agent answers in-scope questions"
            )

        fallback_suggestions = [
            "Increase chunk size in the RAG pipeline to reduce context fragmentation",
            "Expand golden dataset coverage to catch more failure modes",
            "Add regression testing to CI/CD to prevent quality drops",
        ]
